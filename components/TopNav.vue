...
<template>
  <div :class="cn('w-full z-10 bg-white h-[--header-height] flex-shrink-0 flex items-center border-b border-gray-200 dark:border-gray-800 px-4 gap-x-4 min-w-0', className)">
    <div class="flex items-center justify-between flex-1 gap-x-1.5 min-w-0">
      <div class="flex items-stretch gap-1.5 min-w-0">
        <h1 class="flex items-center gap-1.5 font-semibold text-gray-900 dark:text-white min-w-0"><span class="truncate">Explore</span></h1>
      </div>
      <div class="flex items-stretch flex-shrink-0 gap-1.5">
        <UBadge color="white">
          <template v-if="address">
            <NuxtLink :to="'/dashboard/quests'">
              <UButton color="white" variant="ghost">{{ $t('Dashboard') }}</UButton>
            </NuxtLink>
            <span>|</span>
            <UPopover :popper="{ placement: 'bottom-end' }">
              ...
              <template #panel>
                <UButton variant="soft" color="red" @click="() => { doLogout(); reloadNuxtApp() }">
                  Disconnect
                </UButton>
              </template>
            </UPopover>
