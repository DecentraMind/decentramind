...
const { currentUuid, exitCommunity, getLocalCommunity, getCommunityUser, mute, unmute, getMutedUsers } = $(aoCommunityStore())
//let communityInfo = $ref<Awaited<ReturnType<typeof getLocalCommunity>>>()
let communityInfo = $ref<Community>()
...
onMounted(async () => {
  ...
  mutedUsers = await getMutedUsers()
  users = await getCommunityUser(communityInfo.uuid)
  console.log('---------------')
  console.log({users})
})
...
const doMute = async() => {
  isDoingMute = true
  await mute(currentUuid, currentUser)
  isDoingMute = false
...
const doUnmute = async() => {
  isDoingUnmute = true
  await unmute(currentUuid, currentUser)
  mutedUsers = await getMutedUsers()
  isDoingUnmute = false
    ...
    <UPage class="w-full">
      ...
      <UPageGrid class="w-full h-full">
        ...
        <div class="pt-2 pr-10 pl-2">
          ...
          <div v-if="communityInfo" class="pt-2 pl-2">
            <div v-for="user in users" :key="user.address" class="flex items-center justify-between pr-20">
              ...
              >
                ...
                >
                  <UButton
                    v-show="showMuteOrUnmute && communityInfo.owner === address"
